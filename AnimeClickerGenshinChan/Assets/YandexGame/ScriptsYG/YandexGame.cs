...
namespace YG
{
    public class YandexGame : MonoBehaviour
    {
...
#if UNITY_EDITOR
        public void CloseFullAdInEditor()
        {
        ...
        public void CloseFullAd(string wasShown)
        {
            ...
            if (wasShown == "true")
            {
                Message("____Closed Fullscreen Ad");
            }
            else
            {
                if (infoYG.adDisplayCalls == InfoYG.AdCallsMode.until)
                {
                    Message("____The fullscreen ad was not shown! The next time the method is executed to display an ad, the ad will be called because you have selected the method (Until Ad Is Shown)");
                    ResetTimerFullAd();
                }
                else Message("____The fullscreen ad was not shown! The next time the method is executed to display an ad, the ad will not be called, since you have selected the method (Resetting Time After Any Ad Display).");
            }
#endif
        }
        ...
        {
            timerShowAd = 0;
            // print("CloseFullAd TO SHOW");
            CloseFullAd("true");
        }

        public void ResetTimerFullAd()
        {
            // print("ResetTimerFullAd() Fuck");
            // timerShowAd = infoYG.fullscreenAdInterval;
            timerShowAd = 0;
        }
        ...
        public void CloseVideo()
        {
           ...
           // timerShowAd = 0;
            // print($"CLOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOSE: timerShowAd-> {timerShowAd}");
            CloseVideoAd.Invoke();
